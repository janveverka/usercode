"""
parameterizedkeyspdf - The Parameterized KEYS PDF.

This module defines the class ParameterizedKeysPdf which implements a
RooAbsPdf resulting from linear parameterization of the observable
of a RooKeysPdf.

The shape of the PDF is taken from the RooKeysPdf trained on the given
data.  The parameterization introduces dependence on the location
...
##------------------------------------------------------------------------------
class ParameterizedKeysPdf(ROOT.RooKeysPdf):
            ...
            self.xtransform = ROOT.RooFormulaVar(
                ...
                ('{lo} * ({x} <= {lo}) + '
                 ...
                 '{hi} * ({hi} <= {x})').format(
                    ...
                    lo=x.getMin() + 1e-5 * (x.getMax() - x.getMin()),
                    hi=x.getMax() - 1e-5 * (x.getMax() - x.getMin())
                    ),
