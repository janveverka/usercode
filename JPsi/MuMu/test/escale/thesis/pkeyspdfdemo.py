...
def define_workspace_and_variables():
    ...
    '''
    global w, mmgMass, mmMass, phoERes, weight, colors, phoScale, phoRes
...
def beautify_response_title_and_unit():
    ...
    phoERes.SetTitle('E^{#gamma}/E_{true}^{#gamma} - 1')
    phoERes.setUnit('%')
...
def make_scale_dependence_plot():
    ...
    phoRes.setVal(1)
    plot = phoERes.frame(roo.Range(-8, 8))
    latexlabels = []
    ...
    Latex(['r: 1 %',], position=(0.86, 0.8)).draw()
...
def make_resolution_scan_plot():
    ...
    plot.SetTitle('Dependence on Resolution')
    plot.GetYaxis().SetTitle('Probability Density')
    plot.Draw()
    # Latex(cutlabels, position=(0.2, 0.75)).draw()
    Latex(['s: -4, 0, 2 %',], position=(0.75, 0.8)).draw()
    for l in latexlabels:
        l.draw()
