...
#### Take care of Sherpa's extra PDG codes
...
OUTPUT_DIR=$JOB/RAWSIM/sherpack
...
# scp outputGENDIGIHLT.root $T3_URL:$T3_OUTPUT_PATH/$OUTPUT_NAME

# echo "Storing $AFS_OUTPUT_PATH/$OUTPUT_NAME"
# cp outputGENDIGIHLT.root $AFS_OUTPUT_PATH/$OUTPUT_NAME

## Prepare the log.
mkdir log1_${OUTPUT_VERSION}
mv *.log *.py ../python/*.py MIG* *.dat */Run.dat log1_${OUTPUT_VERSION}
tar czf log.tgz log1_${OUTPUT_VERSION}
LOG_NAME=log1_${OUTPUT_VERSION}.tgz
...
## Some extra gymnastics to workaround not being able to use
## +two customization files
STEP2_CFG=step2_RAW2DIGI_RECO.py
cat >> $STEP2_CFG << END_HERE
#### "Randomize" the random seeds
maxSeed = {"HepJamesRandom": 900000000,
  "TRandom3": 900000000,
  "RanecuEngine": 2147483647
...
## Run!
cmsRun $STEP2_CFG 2>&1 | tee -a step2.log

#### Store the step2 output on the T3, AFS and CASTOR
OUTPUT_DIR=$JOB/RECO/sherpack
